...
The mark folds together the two ways Windows lets you prove who you are: two eyes above a stack of nested arcs that is a smile and a fingerprint ridge pattern at the same time — the same strokes carry both readings. The arc count thins out as the icon shrinks, so 16 px stays a clean face while the ridges appear from 24 px upwards.

A smiling face and a fingerprint are both generic motifs, chosen deliberately for that reason. No vendor's logo, wordmark or icon set was used, traced or reproduced.
...
- Replaced the application icon with original artwork generated by [tools/make-icon.ps1](tools/make-icon.ps1). The inherited icon had no traceable origin or license.
